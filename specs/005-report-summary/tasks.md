...
T004 (the `Scan` restructuring, the options struct) and T016 (the tick). `golang-naming` and
`golang-documentation` before T004–T007 and T019 (the exported surface of `internal/report`,
...
`golang-spf13-cobra` before T011–T013 and T017. `golang-error-handling` before T015 (the
joined runtime errors). `golang-safety` and `golang-security` before T011 and T017 (every
log is untrusted input; escape sequences are written to a terminal), and `golang-security`
before T010 (it runs a JDK, sbt and the built binary as child processes). `golang-benchmark`,
then `golang-performance`, before T014 and the benchmark of T017.
...
|---|---|---|
| US1 — the numbers of a run without the HTML report (P1) | T005, T006, T008, T011 | every non-percentile figure of all five corpus runs equals what Gatling recorded; the 3.13.1 golden equals the contract's block; no file is written, no `ko`, no row per request |
| US2 — percentiles that say what they are (P2) | T007, T009, T010, T012 | percentiles pinned per corpus run, 1427 included; every percentile within the rule on the corpus, synthetic and live runs, Gatling 3.11/3.12 as the reference; identical between two reads; the closing line says whose they are; `--percentiles 99.9,90`; bad ranks exit 2 |
| US3 — bands at the engineer's own boundaries (P2) | T013 (over T006) | `--bounds 5,1000` counted against the log's own response times; bad boundaries exit 2 |
| US4 — a run of any size (P2) | T014 | 16 MiB and 256 MiB replays under 32 MiB; a million names cost what one name costs |
| US5 — failures a script can act on (P2) | T015 | zero span, lost outcome, no recorded end, cut short, damaged: each its code and its message |
| US6 — a long read is seen to progress (P3) | T016, T017 | frames under a fake clock; silence off a terminal; standard output byte-identical with and without the block |
...
- **T011** after T005–T010.
- **T012 → T013** after T011; both edit `cmd/galaxio/report.go`.
- **T014** after T007; it touches only tests in `internal/report/` and runs beside T008–T013.
- **T015** after T013. **T016** after T007; **T017** after T015 and T016.
- **T018** after T017. **T019** after T017, beside T018. **T020** after T019. **T021** after
  T018–T020. **T022** last.
...
Phase 6:  T011                         (MVP)
Phase 7:  T012 → T013
Phase 8:  T014                         (beside phases 6 and 7)
Phase 9:  T015
Phase 10: T016 → T017                  (T016 beside phases 7 to 9; T017 after T015)
Phase 11: T018 | T019 → T020 → T021 → T022
```
...
- T014 beside T008–T013 (tests in `internal/report/` only; the command files are not touched).
- T016 beside T012–T015 (`internal/report/source.go` and `scan.go`; the command files are
  not touched until T017).
- T018 and T019 (an integration test file; the README).
...
4. T008–T011: the words, every percentile proved against recorded runs, then **MVP** — the
   summary printed in its final layout at the defaults. Stop here
   and validate US1 with quickstart §1 before going on.
5. T012–T015: the two options, the memory goal, the failures.
6. T016–T017: the progress block, last, because the summary is complete without it.
7. T018–T022: the binary exercised end to end, README, issue #51 amended, gates and
...
- No test compares a percentile Gatling recorded with one this tool prints (FR-019); only
  the reference tests read Gatling 3.11.x and 3.12.x percentiles, to hold them to the rank
  rule, and quickstart §2 lists them.
