#!/usr/bin/env bash
#
# linkage-guard_test.sh — regression suite for .claude/hooks/linkage-guard.sh.
#
# This suite travels with the guard. It is the contract the copies in every
# Galaxio repository share, and each case below is a shape the guard got wrong
# at some point — keep them, and add to them rather than trimming.
#
...
# The suite sits beside the hook it tests, as .githooks/pre-push_test.sh does.
here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
GUARD="${GUARD:-$here/linkage-guard.sh}"
...
run 0 'quoted semicolon mentions a tag' +  'echo "note; git tag v9.9.9"'
run 0 'quoted pipe mentions a tag push' +  'gh pr create --body "note | git push origin v9.9.9"'
...
# parsec diverges here, and the divergence belongs upstream too: publishing is
# what the tag does, and a release branch serves a whole minor line, so its name
# is the wrong version for every patch release in the repository that found it.
run 0 'push a release branch'        'git push origin release/1.0'
run 0 'fast-forward the release branch' 'git push origin origin/main:refs/heads/release/0.0.0'
...
run 2 'tag behind sudo'              'sudo git tag v1.0.7'
run 2 'push behind xargs'            'xargs git push --tags'
run 2 'push behind an env assignment' 'GIT_SSH_COMMAND=ssh git push origin v1.0.7'
run 2 'heredoc first, tag after' \
  "$(printf 'cat > /tmp/notes.md <<%sMD%s\n## v1.0.7\nMD\ngit tag v1.0.7' "'" "'")"

echo "== the version handed to the checker =="
out=$(jq -n --arg c 'git tag v1.0.7' '{tool_input:{command:$c}}' \
      | CLAUDE_PROJECT_DIR="$fake" bash "$GUARD" 2>&1 || true)
if printf '%s' "$out" | grep -q -- '--for-tag v1.0.7'; then
  pass=$((pass + 1)); printf '  ok   [-] the checker is asked about the tag being created\n'
else
  fail=$((fail + 1)); printf '  FAIL the checker was asked: %s\n' "$out"
fi
